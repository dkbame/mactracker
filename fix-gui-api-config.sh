...
# Fix GUI API configuration based on official documentation
# The GUI needs proper NUXT_PUBLIC_API_BASE environment variable
...
print_status "Fixing GUI API configuration based on official documentation..."
...
# 2. Create proper Docker Compose with correct GUI configuration
print_status "2. Creating proper Docker Compose with correct GUI configuration..."
...
print_success "Docker Compose configuration updated with proper GUI settings"

# 3. Clean up databases for fresh start
print_status "3. Cleaning up databases for fresh start..."
...
# 5. Create proper index configuration
...
for i in {1..30}; do
    if curl -s http://127.0.0.1:3001/v1/settings/public | grep -q "tracker\|api"; then
        ...
        print_error "Index failed to start"
...
# Start GUI
print_status "Starting GUI with proper API configuration..."
docker compose -f docker-compose-https.yml up -d gui
sleep 15

# 7. Test all endpoints
print_status "7. Testing all endpoints..."
...
# 8. Show GUI configuration
print_status "8. GUI configuration summary:"
...
print_status "GUI API configuration fix completed!"
print_status ""
print_status "The GUI is now configured with the correct API base URL:"
print_status "https://$DOMAIN/api/v1"
print_status ""
print_status "This should resolve the Index container crashes and 502 errors."
